...
> The denoising autoencoder partially corrupts the initial input vector $\mathbf{x}$ in a stochastic manner: $\tilde{\mathbf{x}} \sim \mathcal{M}_\mathcal{D}(\tilde{\mathbf{x}} \vert \mathbf{x})$.
> The model is then trained to recover the original input.
> $$\begin{aligned}
> \tilde{\mathbf{x}}^{(i)} &amp;\sim \mathcal{M}_\mathcal{D}(\tilde{\mathbf{x}}^{(i)} \vert \mathbf{x}^{(i)})\\
> L_\text{DAE}(\theta, \phi) &amp;= \frac{1}{n} \sum_{i=1}^n (\mathbf{x}^{(i)} - f_\theta(g_\phi(\tilde{\mathbf{x}}^{(i)})))^2
> \end{aligned}$$where $\mathcal{M}_\mathcal{D}$ defines the mapping from the true data samples to the noisy or corrupted ones.
